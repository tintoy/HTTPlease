...
namespace HTTPlease.ValueProviders
{
	...
	public static class ValueProviderExtensions
	{
		...
		/// <summary>
		///		Perform a simple conversion on the value provider.
		/// </summary>
		/// <typeparam name="TContext">
		///		The source type from which the value is extracted.
		/// </typeparam>
		...
		///		A <see cref="ValueProviderConversion{TContext,TValue}"/> whose methods can be used to select the conversion to perform on the value converter.
		/// </returns>
		public static SimpleValueProviderConversion<TValue> ConvertSimple<TContext, TValue>(this IValueProvider<TContext, TValue> valueProvider)
		{
			if (valueProvider == null)
				throw new ArgumentNullException(nameof(valueProvider));

			return new SimpleValueProviderConversion<TValue>(valueProvider);
		}
		...
		///		A <see cref="ValueProviderConversion{TContext,TValue}"/> whose methods can be used to select the conversion to perform on the value converter.
		/// </returns>
		public static SimpleValueProviderConversion<TValue> Convert<TValue>(this ISimpleValueProvider<TValue> valueProvider)
		{
			if (valueProvider == null)
				throw new ArgumentNullException(nameof(valueProvider));
