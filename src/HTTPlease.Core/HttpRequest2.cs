...
namespace HTTPlease
{
	...
	/// </summary>
	public sealed class HttpRequest2
		: HttpRequestBase2, IHttpRequest2, IHttpRequest2<object>
	{
		#region Constants
		...
		/// </summary>
		public static readonly HttpRequest2 Empty = new HttpRequest2(RequestPropertyStores.Immutable());

		/// <summary>
		///		The default factory for <see cref="HttpRequest"/>s.
		/// </summary>
		public static HttpRequestFactory2 Factory { get; } = new HttpRequestFactory2(Empty);
		...
		/// </returns>
		public static HttpRequest2 Create(string requestUri) => Factory.Create(requestUri);
		...
		///		The new <see cref="HttpRequest"/>.
		/// </returns>
		public static HttpRequest2 Create(Uri requestUri) => Factory.Create(requestUri);
		...
		///     The configured <see cref="HttpRequestMessage" />.
		/// </returns>
		HttpRequestMessage IHttpRequest2<object>.BuildRequestMessage(HttpMethod httpMethod, object context, HttpContent body, Uri baseUri)
		{
		...
		///		Actions (if any) to perform on the outgoing request message.
		/// </summary>
		IReadOnlyList<RequestAction<object>> IHttpRequestProperties2<object>.RequestActions => RequestActions;

		/// <summary>
		///		Actions (if any) to perform on the outgoing request message.
		/// </summary>
		IReadOnlyList<ResponseAction<object>> IHttpRequestProperties2<object>.ResponseActions => ResponseActions;

		/// <summary>
		///     The request's URI template parameters (if any).
		/// </summary>
		IReadOnlyDictionary<string, IValueProvider<object, string>> IHttpRequestProperties2<object>.TemplateParameters => TemplateParameters;

		/// <summary>
		///     The request's query parameters (if any).
		/// </summary>
		IReadOnlyDictionary<string, IValueProvider<object, string>> IHttpRequestProperties2<object>.QueryParameters => QueryParameters;
		...
		///		The cloned request.
		/// </returns>
		public new HttpRequest2 Clone(Action<IDictionary<string, object>> modifications)
		{
			if (modifications == null)
				throw new ArgumentNullException(nameof(modifications));

			return (HttpRequest2)base.Clone(modifications);
		}
		...
		{
			return new HttpRequest2(requestProperties);
		}
